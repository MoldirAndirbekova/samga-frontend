...
export default function FlappyBirdGame({ onGameOver, difficulty }: FlappyBirdGameProps) {
  ...
  const createGame = async () => {
    try {
      ...
      console.log("Creating new Flappy Bird game with difficulty:", difficulty);
  ...
  const connectWebSocket = (id: string) => {
    ...
    setGameId(id);
    localStorage.setItem("flappy_bird_game_id", id);
    setSocketConnected(false);
    ...
    const cleanupCallback = () => {
      console.log("Cleaning up Flappy Bird game resources");
    };
     ...
     {gameState.gameActive && !gameState.gameOver && (
       <div className="absolute bottom-4 left-4 right-4 text-center z-50">
         <div className="bg-black bg-opacity-75 text-white p-3 rounded-lg">
           <p className="text-lg">Raise both arms to flap!</p>
         </div>
       </div>
     )}
